...
# Pads lists inside a list with zeros to make it of the same length
...
def tokeniz(fo):
    ...
    vectorizer = CountVectorizer(
        stop_words=my_stop_words, min_df=800)
    bag = vectorizer.fit_transform(fo)
    # print(vectorizer.get_feature_names())
    # print(len(vectorizer.get_feature_names()))
    # print(vectorizer.vocabulary_["2021"])
    return bag.toarray()


docs = np.array(['Mirabai has won a silver medal in weight lifting in Tokyo olympics 2021',
                 'Sindhu has won a bronze medal 2021 in badminton in Tokyo olympics  2021',
                 'Indian hockey team is in top four team in Tokyo olympics 2021 after 40 years  2021'])
